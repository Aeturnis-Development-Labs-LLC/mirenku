...
class AnimeDetailDialog:
    ...
    def create_widgets(self):
        ...
        if self.anime.title_japanese:
            ttk.Label(
                info_frame,
                text=self.anime.title_japanese,
        ...
        self.progress_label = ttk.Label(
            progress_frame,
            text=self.anime.display_progress
        )
        ...
        ttk.Button(
            ...
            text="-",
            width=3,
            command=self.decrement_episode
        ...
        ttk.Button(
            ...
            text="+",
            width=3,
            command=self.increment_episode
        ...
        if self.anime.mal_id:
            mal_frame = ttk.Frame(info_grid)
            mal_frame.grid(row=6, column=0, columnspan=2, sticky='w', pady=(10, 0))
            
            ttk.Button(
                mal_frame,
                text="View on MyAnimeList",
                command=self.open_mal_page
            ).pack(side='left')
            
            if self.mal_service:
                ttk.Button(
                    mal_frame,
                    text="Refresh from MAL",
                    command=self.refresh_from_mal
        ...
        if self.anime.synopsis:
            ...
            # Add scrollbar
            scrollbar = ttk.Scrollbar(synopsis_text)
            scrollbar.pack(side='right', fill='y')
            synopsis_text.config(yscrollcommand=scrollbar.set)
    ...
    def refresh_image(self):
        ...
        try:
            # Force refresh
            new_path = self.image_service.refresh_image(
                self.anime.image_url,
                self.anime.mal_id
            )
