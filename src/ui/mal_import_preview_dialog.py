...
class MALImportPreviewDialog:
    ...
    def import_selected(self):
        ...
        response = messagebox.askyesno(
            "Confirm Import",
            f"Import {len(selected_anime)} anime from MyAnimeList?"
        )
    ...
    def _import_thread(self, anime_list):
        ...
        for i, anime in enumerate(anime_list):
            try:
                ...
                if success:
                    ...
                    if image_url:
                        self.image_service.queue_download(
                            image_url,
                            self.image_service.cache_dir / f"mal_{mal_id}.jpg"
                        )
                else:
                    failed += 1
                ...
                failed += 1
                errors.append(f"Error importing {anime.get('title', 'Unknown')}: {str(e)}")
