...
class MALAuthDialog:
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            main_frame, text="MyAnimeList Authentication", font=("TkDefaultFont", 14, "bold")
        )
        ...
        self.status_label = ttk.Label(
            self.status_frame, text="Checking authentication status...", font=("TkDefaultFont", 10)
        )
        self.status_label.pack()

        self.user_label = ttk.Label(self.status_frame, text="", font=("TkDefaultFont", 9))
...
3. Authorize the app"""
        ...
        info_label = ttk.Label(info_frame, text=info_text, justify="left", wraplength=430)
        info_label.pack()
    ...
    def check_auth_status(self):
        ...
        if self.oauth_client.is_authenticated():
            ...
            self.status_label.config(text="✓ Connected to MyAnimeList", foreground="green")
            ...
            self.authenticated = False
            self.status_label.config(text="✗ Not connected to MyAnimeList", foreground="red")
    ...
    def _fetch_user_info(self):
        ...
        try:
            ...
            if user_data:
                ...
                self.dialog.after(
                    0, lambda: self.user_label.config(text=f"Logged in as: {username}")
                )
            else:
                self.dialog.after(
                    0, lambda: self.user_label.config(text="Unable to fetch user information")
                )
        except Exception as e:
            logger.error(f"Failed to fetch user info: {e}")
            self.dialog.after(
                0, lambda: self.user_label.config(text="Error fetching user information")
            )
    ...
    def start_authentication(self):
        ...
        self.status_label.config(text="Opening browser for authentication...", foreground="blue")
        ...
        try:
            ...
            self.dialog.after(
                ...
                lambda: self.status_label.config(
                    text="Please authorize in your browser...", foreground="blue"
                ),
            )
    ...
    def _auth_complete(self, success: bool, error: Optional[str] = None):
        ...
        if success:
            ...
            self.status_label.config(
                text="✓ Successfully connected to MyAnimeList!", foreground="green"
            )
            ...
            messagebox.showerror(
                "Authentication Failed", f"Failed to connect to MyAnimeList:\n\n{error_msg}"
            )
    ...
    def disconnect(self):
        ...
        if response:
            ...
            self.status_label.config(text="✗ Disconnected from MyAnimeList", foreground="red")
            self.user_label.config(text="")
            ...
            messagebox.showinfo("Disconnected", "Successfully disconnected from MyAnimeList.")
...
class MALAuthManager:
    ...
    def __init__(self, config_dir: Path, client_id: Optional[str] = None):
        ...
        # Use provided client_id, or get from config/prompt user
        ...
        if self.client_id:
            ...
            self.oauth_client = MALOAuth2HTTPClient(self.client_id, token_path)
        else:
    ...
    def is_authenticated(self, silent: bool = False) -> bool:
        """Check if authenticated

        Args:
            silent: If True, don't log errors during token refresh (for startup checks)

        Returns:
            True if authenticated
        """
        return self.oauth_client and self.oauth_client.is_authenticated(silent_refresh=silent)
