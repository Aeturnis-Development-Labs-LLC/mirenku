...
class AddAnimeDialog:
    ...
    def _create_form(self):
        ...
        self.episodes_spin = ttk.Spinbox(
            episode_frame, textvariable=self.episodes_var, from_=0, to=9999, width=10
        )
        self.episodes_spin.pack(side=tk.LEFT)

        ttk.Label(episode_frame, text=" / ").pack(side=tk.LEFT, padx=5)

        self.total_entry = ttk.Entry(episode_frame, textvariable=self.total_var, width=10)
        ...
        self.score_spin = ttk.Spinbox(
            score_frame, textvariable=self.score_var, from_=0, to=10, width=10
        )
        ...
        ttk.Button(button_frame, text="Add", command=self.add_anime, width=15).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Button(button_frame, text="Cancel", command=self.dialog.destroy, width=15).pack(
            side=tk.LEFT, padx=5
        )
...
class EditAnimeDialog:
    ...
    def __init__(self, parent, service: AnimeService, anime: Anime):
        ...
        self.total_var = tk.StringVar(
            value=str(anime.total_episodes) if anime.total_episodes else ""
        )
    ...
    def _create_form(self):
        ...
        ttk.Button(button_frame, text="Save", command=self.save_anime, width=15).pack(
            side=tk.LEFT, padx=5
        )
