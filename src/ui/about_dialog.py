...
class AboutDialog:
    ...
    def create_about_tab(self, parent):
        ...
        # Logo/Title
        title_label = ttk.Label(parent, text="Mirenku", font=("Helvetica", 18, "bold"))
        title_label.pack(pady=(20, 5))

        version_label = ttk.Label(parent, text=f"Version {self.version}", font=("Helvetica", 12))
...
and keep notes on your favorite shows."""
        ...
        desc_label = ttk.Label(parent, text=desc_text, justify="center")
        desc_label.pack(pady=10)

        # Credits
        ttk.Separator(parent, orient="horizontal").pack(fill="x", padx=50, pady=20)

        credits_label = ttk.Label(parent, text="Developed by", font=("Helvetica", 10))
        credits_label.pack()

        company_label = ttk.Label(
            parent, text="Aeturnis Development Labs LLC", font=("Helvetica", 12, "bold")
        )
        ...
        license_label = ttk.Label(
            parent, text="Licensed under Prosperity Public License 3.0.0", font=("Helvetica", 10)
        )
        license_label.pack()
    ...
    def create_diagnostics_tab(self, parent):
        ...
        text_widget = scrolledtext.ScrolledText(
            parent, wrap="word", width=70, height=20, font=("Consolas", 9)
        )
        ...
        diag_info.append(
            f"Running as: {'Executable' if getattr(sys, 'frozen', False) else 'Script'}"
        )
        diag_info.append("")
    ...
    def create_paths_tab(self, parent):
        ...
        for i, (label, path) in enumerate(paths):
            ...
            status_label = ttk.Label(
                frame, text=status, foreground=color, font=("Helvetica", 12, "bold")
            )
