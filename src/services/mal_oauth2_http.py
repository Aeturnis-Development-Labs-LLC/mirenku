...
class MALOAuth2HTTPClient:
    ...
    def _exchange_code_for_tokens(self, auth_code: str) -> bool:
        ...
        """
        try:
            logger.info(f"Exchanging code of length {len(auth_code)}: {auth_code[:10]}... (truncated)")
            logger.info(f"Using redirect_uri: {self.REDIRECT_URI}")
            logger.info(f"Using code_verifier of length {len(self.code_verifier)}: {self.code_verifier[:10]}... (truncated)")
