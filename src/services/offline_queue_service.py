...
class OfflineQueueService:
    ...
    def add_to_queue(
        self, operation: QueueOperation, data: Dict, callback: Optional[Callable] = None
    ) -> int:
        ...
        try:
            ...
            if operation == QueueOperation.SEARCH.value:
                result = self.mal_service.search_anime(
                    data.get("query"), limit=data.get("limit", 20)
                )
                return result is not None

            if operation == QueueOperation.FETCH_DETAILS.value:
                result = self.mal_service.get_anime_details(data.get("mal_id"))
                return result is not None

            if operation == QueueOperation.FETCH_USER_LIST.value:
                result = self.mal_service.get_user_animelist(
                    data.get("username"), status=data.get("status")
                )
                return result is not None

            if operation == QueueOperation.DOWNLOAD_IMAGE.value:
