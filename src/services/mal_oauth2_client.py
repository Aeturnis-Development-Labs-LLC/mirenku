...
class MALOAuth2Client:
    ...
    def _exchange_code_for_tokens(self, auth_code: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Token exchange data: client_id={self.client_id}, code={auth_code[:10]}..., redirect_uri={self.REDIRECT_URI}"
            )
            ...
            try:
                ...
                error_desc = error_data.get(
                    "error_description", error_data.get("message", "No description")
                )
                ...
                if error_msg == "invalid_request":
                    logger.error(
                        "This usually means the redirect_uri doesn't match or PKCE is incorrect"
                    )
