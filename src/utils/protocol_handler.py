...
class ProtocolHandler:
    ...
    def parse_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"scheme": parsed.scheme, "action": action, "params": params, "raw_url": url}
    ...
    def register_oauth_handler(self, callback: Callable):
        ...
        def auth_handler(params):
            ...
            if self.oauth_callback:
                self.oauth_callback(
                    code=code, state=state, error=error or (None if code else "missing_code")
                )
